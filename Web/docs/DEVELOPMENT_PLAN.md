...
| --- | --- | --- | --- |
| M1: Secure foundation | Phase 1 - FastAPI foundation and trusted contract | Establish the secure boundary before any feed is dispatched. | Web provides service credentials, the versioned callback schema, and a test-only fake private-storage seam; Orchestration accepts only authenticated, schema-valid job fixtures and signs a fixture result that Web can verify. |
...
| M2: Feed intake | Phase 2 - Streaming ingestion, mapping, and normalization | Process a persisted CSV after Web has accepted it safely. | Web and Orchestration use the same private R2 bucket per environment: the local-development bucket from `.env.local` or the isolated production bucket from `.env.prod`, with separate least-privilege credentials. Web persists feed metadata, checksum, mapping configuration, and a scoped object key before dispatch; Orchestration reads only that grant, normalizes bounded CSV batches, and returns safe row-level outcomes. |
