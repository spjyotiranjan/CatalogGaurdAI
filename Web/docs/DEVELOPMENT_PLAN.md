...
| M2: Feed intake | Phase 2 - Streaming ingestion, mapping, and normalization | Complete: safely accept a persisted CSV and establish its trusted normalization job. | Web and Orchestration use the same private R2 bucket per environment with separate least-privilege credentials. Web persists immutable metadata/checksum/mapping/job identity, then dispatches a signed v1 job; Orchestration rechecks the granted object, streams CSV rows into a checkpointed operational ledger, and produces safe row-level outcomes. Product/issue application and reliable callback delivery remain M3/M4. |
